...
def start(message):
    ...
    bot.send_photo(message.chat.id, img2, caption=f'Привет, {message.from_user.first_name}. Я - бот, который способен раздеть любую девушку для тебя.'
                                                  '\n'
                                                  f'\n🎁 У тебя есть {gift_token} подарочных монетки! 🎁'
                                                  '\n'
                                                  f'\n🍥 1 монетка = 1 раздевание. Для того, чтобы пополнить баланс, зайди в раздел меню "Пополнить" 🍥', reply_markup=markup)
